...
try:
    if not CLIENT_ID or not CLIENT_SECRET:
        save_file("security_report.json", json.dumps({"error": "Missing StackSpot credentials"}, indent=2))
...
"""
    ...
    if CONVERSATION_ID:
        ...
        print("Nenhum conversation_id informado. A API tentará criar um novo.")
    ...
    print("Resposta agente:", response.text[:3000])
    response.raise_for_status()

    result = response.json()

    save_file("security_report.json", json.dumps(result, indent=2, ensure_ascii=False))
    ...
    if conversation_id:
        save_file("conversation_id.txt", conversation_id)
        print("CONVERSATION_ID:", conversation_id)
    else:
        print("Nenhum conversation_id retornado pela API.")

except Exception as e:
    error_data = {"error": str(e)}
    save_file("security_report.json", json.dumps(error_data, indent=2, ensure_ascii=False))
    print("ERRO:", str(e))
    sys.exit(1)